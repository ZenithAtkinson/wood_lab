...
func printASNtree(node *XMLNode, proc func(string)) {
	...
	doASNtree = func(curr *XMLNode, depth int, comma bool) {
		...
		// just an underscore   - unnamed braces (element of SEQUENCE OF or SET OF structured objects)
		// trailing underscore - unquoted value
		// internal underscore - convert to space
		show := true
		quot := true
		if name == "_" {
			show = false
		} else if strings.HasPrefix(name, "_") {
			// if leading underscore, ignore remainder of name
			show = false
...
func printJSONtree(node *XMLNode, proc func(string)) {
	...
	doJSONtree = func(curr *XMLNode, depth int, comma bool) {
		...
		// just an underscore   - unnamed brackets
		// leading underscore  - array instead of object
		// trailing underscore - unquoted value
		show := true
		array := false
		quot := true
