	...
	xmlPresenter := func(fileNum int, fileName string, out chan<- Plex) {
		...
		PartitionXML("InvDocument", "", false, rdr,
			func(str string) {
				...
				out <- Plex{Which: fileNum, Ident: id, Text: str, Index: 0, Sibs: nil}
			})
	}
		...
		for hp.Len() > 0 {
			...
			if prevIdent == curr.Ident {
				...
				if len(arry) > 0 {
					...
					out <- Plex{Which: 0, Ident: prevIdent, Text: "", Index: rec, Sibs: arry}
...
func CreateFusers(inp <-chan XMLRecord) <-chan Plex {
	...
	go func() {
		...
		for _, id := range keys {
			...
			out <- Plex{Which: 0, Ident: id, Text: "", Index: rec, Sibs: arry}
