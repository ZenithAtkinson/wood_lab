	...
	processElement := func(acc func(string)) {
		...
		for _, stage := range stages {
			...
			sendSlice := func(str string) {
				...
				// apply range reality checks and then extract requested subregion
				getStringSlice := func(str string, min, max int, nonAscii bool) string {

					var rns []rune
					ln := len(str)

					if nonAscii {
						// convert to array of runes for random access by character position
						rns = []rune(str)
						ln = len(rns)
					}
					...
					} else if min <= max && min < ln {
						if max <= ln {
							if nonAscii {
								str = string(rns[min:max])
							} else {
								str = str[min:max]
							}
						} else {
				...
				if min == 0 && max == 0 {
					// no slice extraction requested
				} else if status == NUCLEIC || !IsNotASCII(str) {
					str = getStringSlice(str, min, max, false)
				} else {
					str = getStringSlice(str, min, max, true)
				}
				...
				// send final result to accumulator
				acc(str)
			}
...
func processCommands(
	...
	// allow path component to be specified by a variable with ampersand prefix (undocumented)
	if len(match) > 1 && match[0] == '&' {
		str := match[1:]
		val, ok := variables[str]
		if ok {
			match = val
		}
	}
	...
	if cmds.Position == "" || cmds.Position == "all" {
		...
		if cmds.Position == "first" {
			...
			ExploreNodes(curr, prnt, match, index, level,
				func(node *XMLNode, idx, lvl int) {
					if beg == nil {
						beg = &Limiter{Obj: node, Idx: idx, Lvl: lvl}
					} else {
						end = &Limiter{Obj: node, Idx: idx, Lvl: lvl}
					}
				})
			...
			ExploreNodes(curr, prnt, match, index, level,
				func(node *XMLNode, idx, lvl int) {
					...
					prev = next
					next = &Limiter{Obj: node, Idx: idx, Lvl: lvl}
