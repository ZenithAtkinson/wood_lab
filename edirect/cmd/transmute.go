...
func processOutline(rdr <-chan eutils.XMLBlock, max int) {
	...
	*/
	...
	if len(args) > 0 && args[0] == "-g2x" {
		...
		*/
		...
		psrq := eutils.CreateTextProducer(pattern, "", "", 0, 0, lbsq)
		gbkq := eutils.CreateGBConverters(psrq)
		unsq := eutils.CreateTextUnshuffler(gbkq)

		if lbsq == nil || psrq == nil || gbkq == nil || unsq == nil {
...
`
	...
	if len(args) > 0 && args[0] == "-gbf" {
		...
		psrq := eutils.CreateTextProducer(pattern, require, exclude, 0, 0, lbsq)
		unsq := eutils.CreateTextUnshuffler(psrq)

		if lbsq == nil || psrq == nil || unsq == nil {
	...
	if len(args) > 0 && args[0] == "-txf" {
		...
		lbsq := eutils.CreateTextStreamer(inp)
		psrq := eutils.CreateTextProducer(pattern, require, exclude, min, max, lbsq)
		unsq := eutils.CreateTextUnshuffler(psrq)
	...
	case "-outline":
		...
		if len(args) > 0 {
			mx := args[0]
			val, err := strconv.Atoi(mx)
			if err == nil {
				max = val
			}
		}
		processOutline(rdr, max)
	case "-contour":
		leaf = true
		fallthrough
	case "-synopsis":
